...
export default function MyPayslipPage() {
  ...
  const breadcrumbs = [
    ...
    { key: 'slip', label: 'Payslip', href: `/dashboard/hrm/my-payroll/${params.id}/slip` }
  ]
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <BackButton href={`/dashboard/hrm/my-payroll/${params.id}`} />
          <div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div
          ...
          style={{ backgroundColor: payslip.theme?.themeColor || '#1d4ed8' }}
        >
          <div className="flex items-center justify-between">
            <div>
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="text-gray-600">{payslip.employee?.employeeId}</div>
              <div className="text-gray-600">{payslip.employee?.designation}</div>
            </div>
            <div>
              <div className="text-gray-500">Payroll Period</div>
              <div className="font-semibold text-gray-900">{payslip.period?.name}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                {payslip.earnings?.items?.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-sm">
                    <span>{item.name}</span>
                    <span>PKR {formatAmount(item.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="border-t pt-4 space-y-1 text-sm">
            <div className="flex justify-between"><span>Gross</span><span>PKR {formatAmount(payslip.summary?.grossSalary)}</span></div>
            <div className="flex justify-between"><span>Total Deductions</span><span>PKR {formatAmount(payslip.summary?.totalDeductions)}</span></div>
          ...
          <div
            className="text-xs text-gray-600 border-t pt-3"
            style={{ borderColor: payslip.theme?.accentColor || '#0f172a' }}
          >
