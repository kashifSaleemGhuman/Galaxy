...
export default function EditMachinePage({ params }) {
  ...
  const fetchMachine = async () => {
    try {
      const res = await fetch(`/api/organization/machines/${params.id}`)
      if (!res.ok) throw new Error('Failed to fetch machine')
      const data = await res.json()
      
      setFormData({
        ...data,
        // Ensure null values are empty strings for inputs
        serialNumber: data.serialNumber || '',
        machineId: data.machineId || '',
        name: data.name || '',
        quantity: data.quantity || 1,
        motorDetails: data.motorDetails || '',
        ...
        electricityRating: data.electricityRating || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(msg || 'Failed to update machine')
      }
  ...
  const breadcrumbs = [
    ...
    { key: 'edit', label: 'Edit Machine', href: '#' },
  ]
