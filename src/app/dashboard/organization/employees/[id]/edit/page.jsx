...
export default function EditEmployeePage({ params }) {
  ...
  const fetchEmployee = async () => {
    try {
      const res = await fetch(`/api/organization/employees/${params.id}`)
      ...
      setFormData({
        ...
        dateOfLeaving: formatDate(data.dateOfLeaving),
        salary: data.salary || '',
        // Ensure boolean values are set correctly
        isFirstAider: !!data.isFirstAider,
        isEmergencyResponder: !!data.isEmergencyResponder,
        isFirefighter: !!data.isFirefighter
      })
    } catch (error) {
      console.error(error)
      toast({
  ...
  const breadcrumbs = [
    ...
    { key: 'edit', label: 'Edit Employee', href: '#' },
  ]
