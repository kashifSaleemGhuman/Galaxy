...
export default function DocumentControlRegisterPage() {
  ...
  // Document Control Register data structure
  ...
  const fetchDocumentContent = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/organization/documents/content?documentName=EMS → DOCUMENT CONTROL REGISTER');
      ...
      if (res.ok) {
        if (data.data?.migrationRequired) {
          ...
          setDocumentInfo({
            docNo: data.data.document.docNo || 'ESF-EMS-DCR-10',
  ...
  const handleDeleteRow = (index) => {
    if (registerData.tableData.length > 1) {
      setRegisterData({
        ...registerData,
        tableData: registerData.tableData.filter((_, i) => i !== index)
      });
    }
  };
  ...
  const handleSave = async () => {
    try {
      const res = await fetch('/api/organization/documents/content', {
        ...
        body: JSON.stringify({
          documentName: 'EMS → DOCUMENT CONTROL REGISTER',
          content: registerData,
          changeDescription: 'Updated document control register'
        })
      });
      ...
      setToast({ type: 'success', message: 'Document control register saved successfully. Revision number incremented.' });
    } catch (error) {
      console.error('Error saving document control register:', error);
      if (error.message && (
        error.message.includes('migration') ||
        error.message.includes('does not exist')
      )) {
  ...
  const breadcrumbs = [
    ...
    { key: 'document-control-register', label: 'Document Control Register', href: '#' },
  ];
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">ESF-EMS-DCR-10 - Environmental Management System</p>
            </div>
            <Button
              variant="outline"
              onClick={() => router.push('/dashboard/organization/documents')}
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Document Control Register Table */}
          <div>
                      ...
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <Input
                            ...
                            onChange={(e) => handleTableDataChange(index, 'documentNo', e.target.value)}
                            className="w-full text-sm"
                            placeholder="Enter document no"
                          />
                        ) : (
                          <span className="text-sm">{row.documentNo || '-'}</span>
                        )}
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <Input
                            ...
                            onChange={(e) => handleTableDataChange(index, 'documentName', e.target.value)}
                            className="w-full text-sm"
                            placeholder="Enter document name"
                          />
                        ) : (
                          <span className="text-sm">{row.documentName || '-'}</span>
                        )}
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <Input
                            type="text"
                            value={row.type}
                            onChange={(e) => handleTableDataChange(index, 'type', e.target.value)}
                            className="w-full text-sm text-center"
                            placeholder="Enter type"
                          />
                        ) : (
                          <span className="text-sm text-center block">{row.type || '-'}</span>
                        )}
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <Input
                            ...
                            onChange={(e) => handleTableDataChange(index, 'revisionNo', e.target.value)}
                            className="w-full text-sm text-center"
                            placeholder="e.g., Rev.No-01"
                          />
                        ) : (
                          <span className="text-sm text-center block">{row.revisionNo || '-'}</span>
                        )}
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <Input
                            ...
                            onChange={(e) => handleTableDataChange(index, 'revisionDate', e.target.value)}
                            className="w-full text-sm text-center"
                            placeholder="e.g., 01-01-2024"
                          />
                        ) : (
                          <span className="text-sm text-center block">{row.revisionDate || '-'}</span>
                        )}
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <Input
                            ...
                            onChange={(e) => handleTableDataChange(index, 'responsibility', e.target.value)}
                            className="w-full text-sm"
                            placeholder="Enter responsibility"
                          />
                        ) : (
                          <span className="text-sm">{row.responsibility || '-'}</span>
                        )}
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        {isAdmin ? (
                          <select
                            value={row.status}
                            onChange={(e) => handleTableDataChange(index, 'status', e.target.value)}
                          ...
                          <span className="text-sm text-center block">{row.status || '-'}</span>
                        )}
                      </td>
                      {isAdmin && (
