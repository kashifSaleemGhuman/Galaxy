...
export default function EditCustomerPage({ params }) {
  ...
  const fetchCustomer = async () => {
    try {
      const res = await fetch(`/api/organization/customers/${params.id}`)
      ...
      router.push('/dashboard/organization/customers')
    } finally {
      setLoading(false)
    }
  }
  ...
  const breadcrumbs = [
    ...
    { key: 'edit', label: 'Edit Customer', href: '#' },
  ]
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-sm rounded-lg border border-gray-200 p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <Input required name="name" value={formData.name} onChange={handleChange} placeholder="Customer Name" />
          </div>
        ...
        <div className="flex justify-end space-x-4 mt-8 pt-4 border-t border-gray-200">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push('/dashboard/organization/customers')}
            disabled={saving}
          >
            Cancel
          </Button>
