...
export default function SalesPollingPage() {
  ...
  const checkForUpdates = async () => {
    try {
      ...
      if (isManager) {
        ...
        const approvalsData = await api.get('/api/sales/quotations/approvals', { status: 'pending', limit: 1 });
        const pendingApprovalsList = approvalsData.quotations || [];
        setPendingApprovals(pendingApprovalsList.length);
        
        if (pendingApprovalsList.length > 0 && !isRedirecting) {
          setIsRedirecting(true);
          setRedirectMessage('New quotation approvals pending! Redirecting...');
          setTimeout(() => {
            router.push('/dashboard/sales/approvals');
          }, 2000);
        }
      } else if (isSalesUser) {
  ...
  useEffect(() => {
    if (session?.user?.id && (isManager || isSalesUser)) {
      checkForUpdates();
    }
  }, [session?.user?.id, isManager, isSalesUser]);
  ...
  // Show loading if not a sales user or manager
  if (session?.user?.id && !isManager && !isSalesUser) {
    return (
      ...
      <div className="max-w-2xl w-full bg-white rounded-xl shadow-2xl p-8 border-l-4 border-blue-500">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-3">
            {isManager ? 'Sales Manager Dashboard' : 'Sales User Dashboard'}
          </h1>
          <p className="text-lg text-gray-600 mb-2">
            {isManager 
              ? 'Monitoring for new quotation approvals...'
              : 'Monitoring for quotation status updates...'
            }
          </p>
          <p className="text-sm text-gray-500">
            {isManager
              ? 'You will be automatically redirected when new quotations require approval'
              : 'You will be automatically redirected when your quotations are approved or rejected'
            }
          </p>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-6 mb-8 min-h-[120px] flex items-center">
          <div className="text-center w-full">
            {isManager ? (
              ...
              <>
                <div className="text-2xl font-bold text-blue-900 mb-2">
                  {pendingQuotations} Status Update{pendingQuotations !== 1 ? 's' : ''}
                </div>
                <p className="text-blue-700">
                  {pendingQuotations > 0 
                    ? 'You will be redirected to view your quotations shortly...'
                    : 'No status updates at the moment'
                  }
                </p>
              </>
            )}
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="text-xs text-gray-400 text-center mt-1">
            Role: {isManager ? 'Sales Manager' : 'Sales User'}
          </div>
        </div>
      </div>
    </div>
